...
describe('zipDirectory', () => {
  ...
  describe('#zipDirectory()', () => {
    ...
    beforeEach(() => {
      excludeDevDependenciesStub = sinon.stub(packageService, 'excludeDevDependencies').resolves();
      zipStub = sinon.stub(packageService, 'zip').resolves();
    });

    afterEach(() => {
      packageService.excludeDevDependencies.restore();
      packageService.zip.restore();
    });
    ...
    it('should run promise chain in order', () => {
      ...
      return expect(packageService.zipDirectory(exclude, include, zipFileName)).to.be
        .fulfilled.then(() => {
          expect(excludeDevDependenciesStub).to.have.been.calledOnce;
          expect(zipStub).to.have.been.calledOnce;

          expect(packageService.exclude).to.equal(exclude);
          expect(packageService.include).to.equal(include);
          expect(packageService.zipFileName).to.equal(zipFileName);
        });
    });
  });
  ...
  describe('#excludeDevDependencies()', () => {
    ...
    let execSyncStub;
    const packageJsonFilePath = path.join(serverless.config.servicePath, 'package.json');
    ...
    it('should exclude dev dependencies for services with Node.js runtime', () => {
      ...
      return expect(packageService.excludeDevDependencies()).to.be.fulfilled.then(() => {
        ...
        expect(packageService.exclude).to
          .deep.equal(['node_modules/**']);
        expect(packageService.include).to
  ...
  describe('#zip()', () => {
    ...
    it('should zip a whole service (without include / exclude usage)', () => {
      ...
      packageService.exclude = [];
      packageService.include = [];
      packageService.zipFileName = zipFileName;

      return expect(packageService.zip()).to.eventually.be
